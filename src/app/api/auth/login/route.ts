...
import { runWithDbEnvVar } from '@/lib/db';
...
export const POST = handler(async (req: Request) => {
  ...
  // Which company this login belongs to is decided by the email domain, not
  // by anything in the (not yet existing) session cookie. Everything below,
  // including the failed-login audit entry and the post-login session load,
  // must run against that same tenant's database - the outer handler()'s
  // default context doesn't apply here.
  ...
  return runWithDbEnvVar(dbEnvVar, async () => {
    ...
    cookies().set(SESSION_COOKIE, await createSessionCookie(res.userId, res.dbEnvVar), {
