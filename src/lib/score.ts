...
/** Builds the `entity_id = ANY(...)` / `fy_start_year = ...` / `category_id = ...`
    filter shared by every aggregate below, with placeholder numbers that
  ...
  const [names, scores] = await Promise.all([
    q<{ country_code: string; country_name: string; entities: string }>(
      `SELECT c.code AS country_code, c.name AS country_name,
         ...
         ${category ? 'JOIN compliances cm ON cm.id = o.compliance_id' : ''}
        WHERE o.deleted_at IS NULL AND o.status <> 'Not Applicable'
          AND o.due_date <= CURRENT_DATE ${category ? sql.replace('c.category_id', 'cm.category_id') : sql}
