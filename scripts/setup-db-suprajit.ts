/* ===========================================================================
   SUPRAJIT DATABASE SETUP
   ---------------------------------------------------------------------------
   Suprajit's own tenant database - a straight copy of setup-db.ts pointed at
   DATABASE_URL_SUPRAJIT and Suprajit's own seed data (db/*.suprajit.ts),
   kept as a separate script rather than a flag on the shared one so a typo
   here can never touch LMW's database, and vice versa.

   Usage:
     npm run db:setup:suprajit            apply schema, seed master data and register
     npm run db:setup:suprajit -- --empty seed master data only (no operating activity)
     npm run db:setup:suprajit -- --reset drop everything and rebuild from scratch
...
import { CATEGORIES, COUNTRIES, FEDERAL_LIBRARY } from '../db/library.suprajit';
import { JURISDICTIONS, SUBNATIONAL_LIBRARY, type LibraryItem } from '../db/jurisdictions.suprajit';
import { DIVISIONS, ENTITIES, ROLES, USERS } from '../db/org.suprajit';
...
const CONN = process.env.DATABASE_URL_SUPRAJIT;
if (!CONN) {
  console.error('\n  DATABASE_URL_SUPRAJIT is not set.\n');
  console.error('  1. Add DATABASE_URL_SUPRAJIT to .env.local (a separate Postgres');
  console.error('     connection string from DATABASE_URL - this is Suprajit\'s own database)');
...
async function main() {
  ...
  try {
    ...
    if (RESET) {
      ...
      await client.query(`
        DROP TABLE IF EXISTS score_snapshots, audit_log, notifications, escalation_log, review_actions,
    ...
    console.log("\n  Suprajit's database is ready: entities, users and the full compliance");
    console.log('  library above are seeded. Edit db/org.suprajit.ts / db/library.suprajit.ts');
    console.log('  and re-run this script to change them.');
